 ...
 */
export const Email = z.string().email().min(3).max(64);

/**
 * The password of the user.
 */
export const Password = z.string().min(2).max(512);

/**
 * A clear code sent to the user for one-time access on a specific action / resource. This code is URL safe.
 */
export const ShortCode = z.string().min(1).max(32);
...
export const ListUsersParams = z.object({
  limit: z.number().max(1000).min(1).optional(),
